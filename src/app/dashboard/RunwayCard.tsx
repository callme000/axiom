...
export function RunwayCard({ runwayDays }: RunwayCardProps) {
    ...
    <div className={containerClasses}>
      <div className="flex items-center justify-between mb-4">
        <span className={labelClasses}>
          {isStable ? "Infinite Stability" : "Deterministic Runway"}
        </span>
        {isStable && (
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 bg-emerald-500/40 rounded-full animate-pulse"></span>
            <span className="text-[8px] font-black text-emerald-500/40 uppercase tracking-widest">Active Protection</span>
          </div>
        )}
      </div>
